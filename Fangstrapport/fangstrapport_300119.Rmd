...
if (!require(reshape2)) {
 ...
 if (!require(TeachingDemos)) {
  install.packages("TeachingDemos", repos="http://ftp.ussg.iu.edu/CRAN/")
  library(TeachingDemos)
 }
 if (!require(geosphere)) {
  install.packages("geosphere", repos="http://ftp.ussg.iu.edu/CRAN/")
  library(geosphere)
 }
 if (!require(plotrix)) {
...
# hilsen <- ""
# coast_string <- '../Togtrapporter/maps/europe_simple_wgs84'
# dk_eez_string <- 'kort/eez_dk_line'
...
# coast2 <- shapefile("kort/europe_simple_wgs84.shp")
# coast_df <- fortify(coast2)
# coast_df <- fortify(map("worldHires", xlim=xlim, ylim=ylim, fill=T,plot=F))
# ia <- shapefile("kort/ICES_areas")
# ia_df <- fortify(ia)
# p <- ggplot()+
#   geom_path(data=ia_df, aes(x=long, y=lat, group=group),alpha=.2)+
#   geom_polygon(data=coast_df,aes(x=long,y=lat,group=group),fill="cornsilk")+
#   coord_map(xlim=xlim, ylim=ylim)+
#   # geom_path(data=coast_df,aes(x=long,y=lat,group=group),fill="cornsilk")+
#   geom_segment(data=position,aes(x = lon, xend = lonEnd, y= lat, yend=lonEnd),
#                colour="darkblue")+
#   geom_point(data=position, aes(x=lon,y=lat),col="red")+
#   geom_point(data=position, aes(x=lonEnd,y=latEnd),
#              colour="black",fill="green",pch=24)+
#   geom_text_repel(data=position,aes(x=lon,y=lat,label=station),alpha=.9)+
...
#Samlet fangst per `r catchOperation2`

...
# hilsen <- ""
# coast_string <- "M:/123 - FiskeLine/Rapporter/Rapporter/kort/europe_simple_wgs84"
# dk_eez_string <- "M:/123 - FiskeLine/Rapporter/Rapporter/kort/eez_dk_line"
# ices_areas_string <- "M:/123 - FiskeLine/Rapporter/Rapporter/kort/ICES_areas"
...
# ylim <- c(floor(min(c(position$lat,position$latEnd))/1)*1,
#           ceiling(max(c(position$lat,position$latEnd))/1)*1)
# xlim <- c(floor(min(c(position$lon,position$lonEnd)-.5)/.5)*.5,
#           ceiling(max(c(position$lon,position$lonEnd)+.5)/.5)*.5)
ylim <- c(min(c(position$lat,position$latEnd)-.25),
              max(c(position$lat,position$latEnd))+.25)
xlim <- c(min(c(position$lon,position$lonEnd)-.5),
              max(c(position$lon,position$lonEnd))+.5)
...
# st_geometry(isc2)<-NULL

# isc3 <- isc2 %>% 
#   dplyr::select(ICESNAME,X,Y) %>% 
#   dplyr::mutate(type = "ICES square") 
# colnames(isc3)<-c("label","X","Y","Type")
# colnames(iac2)<-colnames(isc3)
# 
# pos <- position %>% 
#   dplyr::select(station,lonMid,latMid) %>% 
#   dplyr::mutate(type = "Station",
#                 station = as.character(station)) %>% 
#   data.frame()
# 
# colnames(pos) <- colnames(isc3)

# plotlabels <- rbind(isc3,iac2,pos)

# pos <- st_as_sf(position,crs = 4326)
...
p <- ggplot()+
  ...
  geom_text(data = isc2, aes(label = ICESNAME, x=X,y=Y),col="grey60")+
  
  geom_text_repel(data=position,
                  aes(x=lonMid,y=latMid,label=station),alpha=.8,
                  segment.color = "grey50",force = 10)+
  
  # geom_text_repel(data = plotlabels, aes(x=X,y=Y,label=label,col=Type))+
  
  # scale_x_continuous(limits=xlim,
  #                    breaks = seq(from=floor(min(xlim)/.5)*.5,to = max(xlim),by = .5))+
  # scale_y_continuous(limits=ylim,
  #                    breaks = seq(from=floor(min(ylim)/1)*1,to = max(ylim),by = .5))+
  # scale_color_manual(values = c("Start" = "red","Slut"= "black"),

...
# 
# cruise_all <- c("TBM")
# v_cruise <- c("LONNY HEDVIG","Tobisskrabetogt LH.","Tobisskrabetogt R.")
# v_year <- c("2018")
# # # art_out <- c("TBM")
...
# ORDER BY slr.speciesCode", v_year,paste(v_cruise,collapse=("','"))))
# close(channel)
...
path <- ""
V_years <- ifelse(length(v_year) > 1, paste(min(v_year), max(v_year),
                                            sep = '-'), v_year)

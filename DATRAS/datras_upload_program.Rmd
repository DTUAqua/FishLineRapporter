...
hlca_a$sexCode <- ifelse((hlca_a$sexCode == "T" & !is.na(hlca_a$sexCode)), "H", hlca_a$sexCode)
...
if (survey %in% c("MS")){
  ...
  hh1_b <- ddply(hh1_b, c("year", "quarterGearStart", "platform1","gearType","station"), transform, 
...
ca_age <- sqldf('select   sampleId, speciesListId, SpecCodeType, SpecCode,
                ...
                LngtCode, LngtClass, PlusGr, AgeRings, Maturity')
...
# if (survey == "TBM") { hhhlca3$StNo = as.character(hhhlca3$stationName)}
...
if (nrow(ca)>1) {
  ...
  DATA_CA <- data.frame("RecordType"="CA",
                        # "Survey"=hhhlca3$survey[in_CA],
                        "Quarter"=-9,
...
DATA_HL <- data.frame(
  ...
  "HLNoAtLngt" = hhhlca3$HlNoAtLngt[in_HL], 
  # "DateofCalculation"=format(Sys.Date(),"%Y%m%d"),
...
data_hh2 <- data.frame(station = DATA_HH$StNo,
                       ...
                       string = capture.output(
                         write.table(DATA_HH, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)))
data_hl2 <- data.frame(station = DATA_HL$StNo,
                       recordtype = DATA_HL$RecordType,
                       string = capture.output(
                         write.table(DATA_HL, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)))
data_ca2 <- data.frame(station = DATA_CA$StNo,
                       recordtype = DATA_CA$RecordType,
                       string = capture.output(
                         write.table(DATA_CA, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)))
...
# DATA_ALL <- data.frame(string = capture.output(
#     write.table(DATA_HH, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE),
#     write.table(DATA_HL, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE),
#     write.table(DATA_CA, sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)
#     ))

# DATA_ALL$st <- str_detect(DATA_ALL$string, "^(.*?,){,10}")

# data_out <- apply(DATA_ALL, 1, paste0, collapse=",")
data_out <- DATA_ALL$string


# 

# d <- t(data.frame(strsplit(DATA_ALL, ","), check.names = FALSE))
...
#write.table(DATA_HH,file=paste0(path,"old_ALL_",cruise_all,"_",V_years,"_",V_today,".csv"),
#            sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)
#write.table(DATA_HL,file=paste0(path,"old_ALL_",cruise_all,"_",V_years,"_",V_today,".csv"),
#            sep=",", row.names = FALSE, quote=FALSE, append=TRUE, col.names = FALSE)
#write.table(DATA_CA,file=paste0(path,"old_ALL_",cruise_all,"_",V_years,"_",V_today,".csv"),
#            sep=",", row.names = FALSE, quote=FALSE, append=TRUE, col.names = FALSE)

write.table(data_out, file=paste0(path,"ALL_",cruise_all,"_",V_years,"_",V_today,".csv"),
            sep=",", row.names = FALSE, quote=FALSE, col.names = FALSE)

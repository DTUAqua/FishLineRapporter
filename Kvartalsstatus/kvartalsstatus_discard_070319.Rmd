...
stat_input <- sample %>% 
  filter(if(quarter<5) qtr==quarter else !is.na(qtr)) %>% 
  select(trip,station,lat,lon,latT,lonT,dfuArea,statisticalRectangle)
...
gear_input[gear_input$dfuArea %in% c(21) & 
             !(gear_input$selectionDevice %in% c("Seltra 300","Topløs/seltra >175mm")) & 
             gear_input$gearType %in% c("OTB","OTT","PTB"),"dfuArea"] <- 
  paste("\\color{red}",gear_input[gear_input$dfuArea %in% c(21) &
                                    !(gear_input$selectionDevice %in% c("Seltra 300","Topløs/seltra >175mm")) & 
                                    gear_input$gearType %in% c("OTB","OTT","PTB"),"dfuArea"])
...
gear_input[gear_input$dfuArea %in% c(22:32) & 
  ...
  paste("\\color{red}",gear_input[gear_input$dfuArea %in% c(22:32) & 
                                    !(gear_input$selectionDevice %in% c("110 Bacoma","120 Bacoma","T90")) & 
                                    gear_input$gearType %in% c("OTB","OTT","PTB"),"dfuArea"])
...
for (i in 1:nrow(data1t)) {
    data1t$tjek[i] <- is.na(select(data1t[i,],contains(data1t$trin[i]))) & is.na(data1t$raisingFactor[i]) &
...
if (quarter<5){
  data <- speciesList_a[!(speciesList_a$station %in% unique(statError$station)) &
...
for (i in spec_ind) {
   # dat_spec <- animal_ind2[animal_ind2$speciesCode==i,]
    ...
    #lim <- ifelse(nrow(db_spec) < 100, 5, ifelse(nrow(db_spec) < 1000, 8, 12 ))
     lim <- ifelse(nrow(db_spec) < 10,nrow(db_spec),ifelse(nrow(db_spec) < 100, 5, 10))

     db_spec[is.nan(db_spec$cooksd),"cooksd"] <- 1

      db_spec[db_spec$cooksd >= db_spec$cooksd[order(db_spec$cooksd, decreasing=TRUE)][lim] ,"diffId"] <-
        1:nrow(db_spec[ db_spec$cooksd >= db_spec$cooksd[order(db_spec$cooksd, decreasing=TRUE)][lim] ,])

      if (quarter<5){
        titleString <- sprintf(" Art: %s - Kvartal: %s",i,quarter)
      }else{
        titleString <- sprintf(" Art: %s - År: %s",i,year)
        }

    p_individ <- ggplot(data=db_spec, aes(x=length, y=weight, label=station)) +
                        geom_point(data=db_spec,aes(color="all"),pch=18,size=5)+
                        geom_point(data=db_spec[db_spec$year==year,],aes(color="qtr"),pch=18,size=5)+
                       ...
                       #       panel.border = element_rect(colour = "black", fill=NA, size=1))+
                        ...
                        scale_color_manual(values=c("cadetblue3","red"),name="",labels=c("Øvrig database","Valgt kvartal og år"))+
                        theme(plot.margin=unit(c(0.5,0,0.5,0.5), "cm"))+
                       #geom_text(aes(label=ifelse(cooksd>lim*mean(cooksd, na.rm=T),as.character(station),'')),hjust=0.5,vjust=0)
   ...
   #   db_diff <- rbind(db_diff,db_spec[db_spec$cooksd>lim*mean(db_spec$cooksd, na.rm=T),c(1:8)])

            db_diff <- rbind(db_diff,db_spec[!is.na(db_spec$diff),c(1:4,12,5:8)])
...
if (nrow(db_diff)>0) {
      ...
      db_diff <- db_diff[order(db_diff$speciesCode,db_diff$year,db_diff$trip,db_diff$station,db_diff$landingCategory,db_diff$individNum),]
...
five_year_query <- paste("SELECT [year],[cruise],[trip],[station],[quarterGearStart] as qtr,[speciesCode],[landingCategory],[individNum],[number],[length],[age]
...
length_age <- function(dataset, include_individNum = TRUE){
      ...
      dataset[dataset$year==year,"year_group"] <- printQtr("Valgt kvartal og år",
                                                           "Valgt år")
      dataset[dataset$year!=year,"year_group"] <- "Øvrig database"
        ...
        for (i in spec_AL) {
            ...
            # # creates data for plotting outliers in custom colours
             db_spec <- ddply(db_spec0, .(age),
                              ...
                              outlier =
                                ifelse(test = (length<lower.limit | length > upper.limit),
            ...
            # lim <- 10
            #
            # db_spec$cd <- cooks.distance(lm(log_length ~ age,
            #                                 data = db_spec[db_spec$year==year,]))*db_spec$age
            #
          ...
          p_ind <- ggplot(data=db_spec, aes(x=factor(age), y=length, label=outlier)) +

                # stat_boxplot(geom ='errorbar')+
                # geom_boxplot(data=db_spec)+

            geom_violin()+

                # geom_jitter(data=db_spec[db_spec$year!=year,],aes(color="all"),pch=20,size=5,width=0.2, height=0)+
                # geom_jitter(data=db_spec[db_spec$year==year,],aes(color="qtr"),pch=20,size=5,width=0.2, height=0)+
            geom_point(data=db_spec,aes(color=year_group),pch=18,size=5)+
               # geom_point(data=db_spec[db_spec$year==year,],aes(color="qtr"),pch=18,size=5)+
            theme_minimal(base_size = 18)+
            ...
            theme(axis.title.y = element_text(vjust = 2),
               ...
               # scale_color_manual(values=c("all"="cadetblue3","qtr"="red"), name = "",
                #                  labels=c("Øvrig database","Valgt kvartal og år"))+
                            # scale_x_discrete(limits = as.factor(0:max(db_spec$age)),
                            #                  labels = c(0,as.factor(0:max(db_spec$age))),
                            #                  expand = waiver())+
                            # geom_text_repel(aes(label=as.character(diffId)),hjust=-0.5,vjust=0.4)+
                geom_text_repel(aes(label=outlier),hjust=-0.5,vjust=0.4)+
              coord_flip()
              ...
              db_diff <- rbind(db_diff,db_spec[!is.na(db_spec$outlier),

...
y1 <- 2021
y2 <- 2021
cruise <- c("BITS-2", "IESNS", "IN-LYNG")
areas <- c(20:26, "2A", "4A", "4B")
species <- c("SIL", "TOR", "RSP")
```
...
age[!(is.na(age$otolithReadingRemark) & is.na(age$age)), ]
...
fun <- function(k) {
  ...
  anim1 <- animal[animal$speciesCode == species[k], ]
  ##subset by area
  areas <- unique(fish$area)
  fun <- function(kk) {
  
    data <- fish[fish$area == areas[kk], ]
    anim <- anim1[anim1$area == areas[kk], ]
    ##subset by age
    ID <- unique(data$age)
    fun <- function(kkk) {
    ...
    plot(old$log_length, old$log_weight, pch = 16, cex = 0.1, 
            main = paste(paste0(species[k], " - Area ", areas[kk], " - Length at Weight")),
            xlab = "Log(Længde) (mm)",
            ylab = "Log(Vægt) (kg)", 
            cex.main = 1.5,
            cex.lab = 1.5)+ 
...
age_outliers <- outliers[outliers$type == "age", 
                         c("animalId", "speciesCode", "cruise", "date", "station", "area",
                                         "sexCode", "length_mm", "age")] #state the variable that should be

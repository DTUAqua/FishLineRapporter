...
age <- sqlQuery(channel, paste("SELECT   animalId, individNum, year, 
                                     ...
                                     WHERE    (speciesCode IN ('", paste(species,collapse = "','"),"')) AND 
                                              ...
                                              (quarterGearStart IN ('", paste(qtrs,collapse = "','"),"'))
...
animal$weight <- animal$weight/animal$number
...
age_fish <- merge(age, merged, 
                  by = c("animalId" ,"year", "cruise","trip", "station", "speciesCode"), all.x = T)
...
if (nrow(age_fish) != 0){
    ...
    fun <- function(kk) {
      ...
      anim <- anim[!is.na(anim$weight) & anim$weight > 0, ]

...
rm(list=ls())
prefix <- ""
setwd("Q:/mynd/RekreativtFiskeri/REKREA/Rekrea-programs/Rekrea-programs")
###############################################################################
...
p_load(grid,stringr,directlabels,gridExtra,scales,ggplot2,plyr,dplyr,knitr,xtable,
       data.table,tidyr,xlsx,lubridate,zoo,uuid,stringi,RODBC)
###############################################################################
...
TRIP <- trip[trip$Status=="Complete",-2] %>% 
    group_by(f.spm1_1) %>% 
    mutate(Response.ID = paste0("TR",Response.ID),
            a.spm1_1 = 1:n(),
...
sgIds <- OFS2 %>% 
  select(Response.ID,TripRespId)

CS3 <- CS2 %>% 
  # select(-Response.ID) %>% 
  # left_join(sgIds)
  mutate(Response.ID = ifelse(f.spm1_1=="116" |
...
age_data1 <- age_data#bind_rows(age_data_sgId,age_data_noSgId1) 


catch_age_data <- merge(catch_data,age_data1,all=TRUE)
...
no_catch_tor <- no_catch_tor0 %>%
    ...
    mutate(respNum = as.numeric(respNum),
           ...
           endHourInt = startHourTrip, 
           endMinInt = startMinTrip
           ) %>%
  mutate(dateGearStart = as.numeric(as.POSIXct(
               sprintf("%s %s:%s:00",
                       as.Date(date, format="%d/%m/%Y"),
                       startHourTrip,startMinTrip),
               origin="1970-01-01")),
           dateGearEnd = as.numeric(as.POSIXct(
               sprintf("%s %s:%s:00",
                       as.Date(date, format="%d/%m/%Y"),endHourInt,endMinInt),
               origin="1970-01-01"))) %>%
    select(-startHourTrip,-startMinTrip,-endHourInt,-endMinInt)
               
catch_age_data2 <- bind_rows(catch_age_data, no_catch_tor)
...
ca_data_out$dateGearStart <- as.POSIXct(ca_data_out$dateGearStart,
                                        origin="1970-01-01",tz = "UTC")
ca_data_out$dateGearEnd <- as.POSIXct(ca_data_out$dateGearEnd, 
                                      origin="1970-01-01",tz = "UTC")
...
# resp_complete$tripId <- gsub("^NA-(.+)","1-\\1",resp_complete$tripId)
# ca_data_out$tripId <- gsub("^NA-(.+)","1-\\1",ca_data_out$tripId)
# trip_data_out$tripId <- gsub("^NA-(.+)","1-\\1",trip_data_out$tripId)
...
# for (i in 1:nrow(catch_db_out)){
#     catch_db_out$dateGearStart[i] <- ifelse(is.na(catch_db_out$dateGearStart[i]),
#                                      tripdates$dateGearStart[tripdates$tripId==catch_db_out$tripId[i]],
#                                      catch_db_out$dateGearStart[i])
# }
...
if (checkMissingValues(catch_db_fiskeline1[,c("sgId","respNum","sampleType",
    ...
    write.csv2(catch_db_fiskeline1,
...
if (checkMissingValues(trip_db_out[,c("sgTRid","tripId","tripNum")])==0){
  ...
  write.csv2(trip_db_out, #%>% filter(year==2019),
...
# file to be used by Farivar when he enters ages.
# He should send it back by email whenever there are new ages
# Note the different datestamp used to avoid confusion with catch file in normal output dir
# write.xlsx(catch_db_out,
#           paste0("J:/Indsam\ 2018/LYNGBY/LYSTFISKERI/REKREA_COD23_CATCH/",
#                  "REKREA_CATCH_COD23_",v_today_ymd,".xlsx"),
#           row.names = FALSE, col.names = T,showNA = F)

# View(catch_db_fiskeline1 %>% filter(is.na(sgId)))

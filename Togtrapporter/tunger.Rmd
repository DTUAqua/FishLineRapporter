...
p_load(sqldf,RODBC,xtable,ggplot2,lubridate,reshape2,ggrepel,grid,gridExtra,
       gplots,Hmisc,stringr,dismo,raster,marmap,rgdal,maptools,rasterVis,shapefiles,
       calibrate,plotrix,base,mapplots,data.table,tinytex,knitr,marmap,sf,raster,
       colormap,dplyr,wesanderson)
...
query4<-paste("SELECT Sample.cruise,Sample.trip,Sample.station, Sample.stationName, Sample.fishingtime, Sample.latPosStartDec AS lat, Sample.lonPosStartDec AS lon,
...
cruise_title2 <- ifelse(substr(cruise,1,4)=="IBTS",
                        "International Bottom Trawl Survey",
                        ifelse(substr(cruise,1,4)=="BITS",
                                                                                              "Baltic International Trawl Survey",""))
cruise_title <- ifelse(substr(cruise,1,4) %in% c("IBTS","BITS","KASU"),
                       paste0(cruiseName," ",Quarter[1],"Q"," ",year),
                       cruiseName)
...
colnames(participants)[3] <- sprintf("Function \\\\ and tasks")
colnames(participants) <- sprintf("\\thead{ %s } ",colnames(participants) )

for (i in sort(unique(participants$Leg))){
 cat(sprintf("\\paragraph{Leg %s} ",i))
 
mylatex(participants[participants$Leg==i,1:3],
       file='',
       booktabs =T,
       col.just = c(rep("p{4cm}",3)), 
       # rgroup =n.rgroup$rgroup ,
       rowname=NULL,
       # n.rgroup=n.rgroup$n.rgroup,
       # rowlabel="",
...
bat <- marmap::getNOAA.bathy(lon1 = 6, lon2 = 14,lat1 = 53, lat2 = 60, resolution = 1,keep = T)
# bat2 <- fortify(bat)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis',fig.width=7,fig.pos='h',fig.cap=paste0("Survey map with all sampling stations.\\label{fig:mapall}")}

stationMap <- function(data,x,y,labels="",cpue=""){
  ...
  # d2 <- st_as_sf(rasterToPolygons(d))
  # d <- bat2
  
  # upper and lower elevation bounds
  z <- max(d$layer, na.rm=T)
  a <- min(d$layer, na.rm=T)
  
  # breaks and labels for color scale
  brks <- c(a, 0, -15, -30, -50, -75, -100, -150, z)
  ...
  # d2 <- d[d$x >= min(x,na.rm=T) & d$x <= max(x,na.rm=T) & 
  #          d$y >= min(y,na.rm=T) & d$y <= max(y,na.rm=T),]
  ...
  p <- ggplot()# +
    ...
    geom_raster(data = d, aes(x, y, fill = bin)) 
  
    if (cpue == ""){
     p <- p +  geom_point(data = data,aes(x=lon,y=lat,col=platform1))
    }
    
    # geom_sf(data = dkeez, color="red")+
    p <- p + geom_sf(data= kyst,fill="#555555ff")+
    ...
    if (labels!=""){
      p <- p + geom_label_repel(data = data, 
                                aes(x=lon,y=lat,label=stationName),size=3)+
        labs(title = sprintf("Area: %s",unique(data$dfuArea)))
    }
    ...
    if (cpue!=""){
      p <- p + geom_point(data = data, 
        ...
        labs(title = sprintf("CPUE: %s",unique(data$speciesCode)))
    }
  ...
  p <- p +
    ...
    theme_minimal() +
    labs(fill = "Elevation (m)",y="Latitude",x="Longitude",col="Ship")
...
stationMap(data = pos,x = xlim2,y = ylim2)
...
for (a in as.character(sort(unique(pos$dfuArea)))){
  ...
  # if (any(pos$dfuArea==a)) {
    ...
    #area 20
    ylim2 <- c(floor(min(pos2$lat,na.rm=T)/.1)*.1,ceiling(max(pos2$lat,na.rm=T)/.1)*.1)
    xlim2 <- c(floor(min(pos2$lon,na.rm=T)/.5)*.5,ceiling(max(pos2$lon,na.rm=T)/.5)*.5)
    ...
    colnames(pos3)<-c("Station \\\\ Name","Gear \\\\ Type", "Gear \\\\ Quality",
                      "Latitude", "Longitude","Ship")
    colnames(pos3) <- sprintf("\\thead{ %s } ",colnames(pos3))

    mylatex(pos3,
           file='',
           booktabs =T,
           rowname=NULL,
           col.just = c(rep("l",ncol(pos3))), 
           # rgroup =n.rgroup$rgroup ,
           
           # n.rgroup=n.rgroup$n.+rgroup,
           # rowlabel="",
           # colnamesTexCmd="thead",
           # rgroupTexCmd="itshape",
           longtable=T,
           where='H',
           collabel.just=c(rep("l",ncol(pos3))),
           center='none')
...
tng <- sqlQuery(channel, sprintf("SELECT slr.[year]
  ...
  slr.station = sam.station
    where slr.cruise like 'tunge%s' and slr.year = %s
  and slr.speciesCode in ('tng','tor','rsp','dvh')
  ","%",year),stringsAsFactors=F)
close(channel)

tng$cpueNum <- tng$numberTotal*60/tng$fishingTime
tng$cpueWeight <- tng$weightTotal*60/tng$fishingTime
...
All catches are standardized to 1 km^2 swept prior to further calculations.
...
query1<-paste0("SELECT    AnimalRaised.trip, AnimalRaised.year, AnimalRaised.speciesCode, AnimalRaised.dfuArea,
              ...
              FROM       AnimalRaised
              WHERE      (AnimalRaised.year = ",year," and
...
```{r, echo=FALSE, message=FALSE, warning=FALSE,results='asis', fig.width=7, fig.height=4,fig.cap=paste0("Sole length distribution per area for ",cruise,", ",year), fig.pos="H"}
...
speciesDat <- data_TNG %>% 
 ...
 plot1  <- p + geom_bar(stat="identity",aes(fill=factor(dfuArea)), position ="dodge")+
    ...
    labs(x = "Length (mm)", y="Number",title=spec)+
...
#   
#   plot(bat, image=T, land=T,
#            deepest.isobath = -150,shallowest.isobath = -15,
#            step = 25,drawlabels = T,
#            xlab=expression("Longitude"*~degree*"E"),
#            ylab=expression("Latitude"*~degree*"N"),
#            bpal = list(c(min(bat), 0, blues)))
# 
...
#           pch=c(16, 16,16), col = c("purple","darkgreen","dark orange"),
#           cex = 0.9)
```

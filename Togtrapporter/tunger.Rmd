...
for (i in sort(unique(participants$Leg))){
  ...
  participants2 <- participants[participants$Leg==i,1:3]
  
  colnames(participants2) <- sprintf("\\thead{ %s } ",colnames(participants2))
...
stationMap <- function(data,x,y,labels="",cpue="",titleString = "",filename=""){
  ...
  if (filename!=""){
    # png(filename = filename,width = 10,height = 11,pointsize = 200)
    # plot(p)
    # dev.off()
...
for (a in as.character(sort(unique(pos$dfuArea)))){
  ...
  # if (any(pos$dfuArea==a)) {
    ...
    stationMap(data = pos2,x = xlim2,y = ylim2, labels="yes",
               ...
               filename = sprintf("%s-all_stations_in_area_%s-%s.png",
...
stationMap(data = tng3, x = xlim2, y = ylim2, cpue="yes",
           filename = sprintf("%s-all_stations-cpue-%s.png",
                              gsub(" ", "_",cruise_title),gsub("-","",Sys.Date())))

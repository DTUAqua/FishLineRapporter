...
str_oxygen <- ""#@paramOxygen
str_comments <- ""#@paramComments
setwd("M:/123 - FiskeLine/Rapporter/Togtrapporter")
```
...
if (nrow(pos>0)){
  if (getwd() == "M:/123 - FiskeLine/Rapporter/Togtrapporter"){
    coast <- read.shapefile('maps/europe_simple_wgs84')
    # dk_eez <- read.shapefile('maps/eez_dk_poly.shp')
  } else{
  ...
  min_lon <- min(pos$lon, na.rm = T)
  max_lon <- max(pos$lon, na.rm = T)
  min_lat <- min(pos$lat, na.rm = T)
  max_lat <- max(pos$lat, na.rm = T)
  ...
  basemap(xlim=xlim, ylim=ylim, 
          main = "",xlab=expression("Longitude"*~degree*"E"),
          ylab=expression("Latitude"*~degree*"N") ) #, bg="white")
  ...
  pointLabel(x=pos[pos$gearQuality=="V","lon"], y=pos[pos$gearQuality=="V","lat"],
       labels=pos[pos$gearQuality=="V","stationName"],cex=4,offset=-1,
  ...
  points(pos[pos$gearType=="OTB" & pos$gearQuality=="V","lon"], 
         pos[pos$gearType=="OTB" & pos$gearQuality=="V","lat"], pch=16, 
         col="blue", cex=.8)
  points(pos[pos$gearType=="TV3" & pos$gearQuality=="V","lon"], 
         pos[pos$gearType=="TV3" & pos$gearQuality=="V","lat"], pch=16, 
         col="darkgreen", cex=.8)
  points(pos[pos$gearQuality=="I","lon"], pos[pos$gearQuality=="I","lat"], 
  ...
  cat("No stations were tracked on this cruise.")
  }
...
if (substr(cruise,1,4)=="IBTS") {
      ...
      #RF_square <- read.table("X:\\Line\\Togtrapport\\join_RFareas_ICESareas_selected.txt",sep="",header=TRUE)
            ...
            RFS1 <- c("48E6","49E6","50E6","51E6","52E6","48E7","49E7","50E7",
                      "51E7","52E7","47E8","48E8","49E8","50E8","51E8","52E8",
                      "47E9","48E9","49E9","50E9","51E9","52E9","44F0","45F0",
                      "46F0","47F0","48F0","49F0","50F0","51F0","52F0","44F1",
                      "45F1","46F1","47F1","48F1","49F1","50F1","51F1","52F1",
                      "45F2","46F2","47F2","48F2","49F2","50F2","51F2","52F2",
                      "45F3","46F3","47F3","48F3","49F3","50F3","51F3","52F3",
                      "45F4","40F0","41F0","42F0","43F0","39F1","40F1","41F1",
                      "42F1","43F1","39F2","40F2","41F2","42F2","43F2","44F2",
                      "40F3","41F3","42F3","43F3","44F3","40F4","41F4","42F4",
                      "43F4","44F4","41E6","42E6","43E6","44E6","45E6","46E6",
                      "47E6","41E7","42E7","43E7","44E7","45E7","46E7","47E7",
                      "41E8","42E8","43E8","44E8","45E8","46E8","41E9","42E9",
                      "43E9","44E9","45E9","46E9","40E6","40E7","38E8","39E8",
                      "40E8","36E9","37E9","38E9","39E9","40E9","36F0","37F0",
                      "38F0","39F0","37F1","38F1","31F0","32F0","34F0","35F0",
                      "31F1","32F1","33F1","34F1","35F1","36F1","33F2","34F2",
                      "35F2","36F2","31F2","32F2","37F2","38F2","31F3","32F3",
                      "33F3","34F3","35F3","36F3","37F3","38F3","39F3","31F4",
                      "32F4","33F4","34F4","35F4","36F4","37F4","38F4","39F4",
                      "33F5","34F5","35F5","36F5","37F5","38F5","39F5","35F6",
                      "36F6","37F6","38F6","39F6","35F7","36F7","37F7","38F7",
                      "39F7","35F8","36F8","37F8","38F8","39F8","35F9","36F9",
                      "40F5","41F5","42F5","43F5","44F5","40F6","41F6","42F6",
                      "43F6","40F7","41F7","42F7","43F7","40F8","41F8","42F8",
                      "43F8","44F8","45F8","46F8","47F8","48F8","43F9","44F9",
                      "45F9","46F9","47F9","48F9","44G0","45G0","46G0","47G0",
                      "48G0","44G1","45G1","46G1","47G1","48G1","41G0","42G0",
                      "43G0","41G1","42G1","43G1","41G2","42G2","43G2")
      RFS2 <- c(rep(1,57),rep(2,25),rep(3,26),rep(4,16),rep(5,14),rep(6,46),
                rep(7,16),rep(8,22),rep(9,9))
      RF_square_val <- data.frame("ICESsquare"=RFS1,"Rfarea"=RFS2)
      RF_square <- RF_square_val

      single_fish <- merge(data_animal[!is.na(data_animal$individNum),],
                           RF_square, by="ICESsquare", all.x=TRUE)
      single_fish <- merge(single_fish,species_names, by="speciesCode", all.x=TRUE)

      single_fish2 <- ddply(single_fish, 
                            .(Species,Rfarea),
                            summarize, 
                            Tot_num=sum(number))
      ...
      RFAfish <- c("Herring","Sprat","Cod","Haddock","Whiting","Mackerel",
      ...
      rgroup <- c("Stratified by roundfish area",
                  "Not stratified by roundfish area","All species")
...
if (cruise2 %in% c("BITS","Torsk","Tunger")) {
 ...
 speciesDat <- ddply(data_TORTNG[data_TORTNG$year==year &
                                   data_TORTNG$speciesCode==spec,],
                     .(year,length,dfuArea),
                     summarize,
                     number=sum(numberTotalPerLength))

 p <- ggplot(speciesDat, aes(x=length, y=number))+
   geom_bar(stat="identity",aes(fill=factor(dfuArea)), position ="dodge")+
   ...
   theme(axis.title.y = element_text(vjust = 2),
         ...
         legend.background = element_rect(size=0.6, linetype="solid", 
                                          colour ="black"))+
   scale_x_continuous(breaks = seq(100, round_any(max(speciesDat$length),100, 
                                                  f = ceiling), by = 100))

...
sandeelAreas <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/Tobis/Tobisomr.shp"
sandeelBanksString <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/Tobis/2018_opdatering/Tobisbanker2018.shp"
...
sea <- st_read(sandeelAreas,stringsAsFactors = F) %>% 
  st_transform(4326)
# banker <- st_read()
...
sandeelBanks <- st_read(sandeelBanksString)
```
...
stationMap <- function(data,x,y,labels="",cpue="",titleString = "",filename=""){
  ...
  p <- p + 
    geom_sf(data = dkeez, aes(linetype = "DK"),color="red",fill=NA)+
    geom_sf(data= kyst,fill="#555555ff")+
    geom_sf(data = sandeelBanks, fill = "#999999",alpha = .8)+
    coord_sf(xlim = x, ylim = y,
               datum = st_crs(4326))
  
   if (cpue == ""){
     
     shipColors <- c('#e41a1c','#4daf4a','#984ea3','#ff7f00',
                            '#ffff33','#a65628','#f781bf','#377eb8')
            
     for (i in 1:length(levels(pos$platform1))){
     
       p <- p + geom_point(data = data[data$platform1==levels(data$platform1)[i],],
                           aes(x=lon,y=lat),size=3,
                           shape = 21, col = "black", fill = shipColors[i])
     }
    }

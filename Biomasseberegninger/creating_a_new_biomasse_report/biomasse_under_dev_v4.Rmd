...
ref_sample <- distinct(read.csv(paste0(output_dir, "ref_table_filled_v2.csv")), gearType, gearVal, stationType, redsk_dim, Befisket_Areal_m2_x, Befisket_Areal_m2_y, Befisket_Areal_m2_fun)
...
data <- aggregate(data, c(Befisket_Areal_m2_x, Befisket_Areal_m2_y), FUN = data$Befisket_Areal_m2_fun)

data <- mutate(data, Befisket_Areal_m2 = ifelse(!(stationType %in% c('RAM','VV')), redsk_dim*DISTANCE,
                                                ifelse(stationType %in% c('RAM','VV'), redsk_dim, NA)))
...
data <- mutate(data, areal_metode = ifelse(!(data$stationType %in% c('RAM','VV','SUG')), DISMETH,
                                             ifelse(stationType == "RAM", "Ramme-areal", 
                                                    ifelse(stationType == "VV", "Grab-areal", 
                                                           ifelse(stationType == "SUG", "Suge-areal", NA)))))
```

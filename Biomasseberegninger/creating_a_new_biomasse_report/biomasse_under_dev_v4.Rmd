...
v3: Distance is calculated seperatly - no changes. All station with relevant New_DISMETH == "Kan ikke udregnes" are invalid. Second version of v3 only includes valid hauls like in the old report.

v4: Reference tabel is used for calculating BEFISKM2 - no changes
...
ref_sample <- distinct(read.csv(paste0(output_dir, "ref_table_filled_v2.csv")), gearType, gearVal, stationType, redsk_dim, 
                       Befisket_Areal_m2_x, Befisket_Areal_m2_y, Befisket_Areal_m2_fun)
...
tab_dat <- summarise(group_by(data, stationType, DISMETH), noSamples = length(unique(sampleId)))
makeTable(tab_dat)

```

# Befisket areal (BEFISKM2 - omdøbes senere)
...
fak <- factor(data$Befisket_Areal_m2_fun)
...
for (i in levels(fak)) {
  ...
  dat_sub$test <- paste0("(dat_sub$", gsub("\\*", "\\ * dat_sub$", dat_sub$Befisket_Areal_m2_fun), ")")
  
  eval(parse(text = paste0("dat_sub$Befisket_Areal_m2 <- ", dat_sub$test, "")))
...
data <- mutate(data, areal_metode = ifelse(!(data$stationType %in% c('RAM','VV','SUG')), DISMETH,
                                             ifelse(stationType == "RAM", "Ramme-areal",
                                                    ifelse(stationType == "VV", "Grab-areal",
                                                           ifelse(stationType == "SUG", "Suge-areal", NA))))) # This is empty info

dat_tab <- summarise(group_by(data, gearType, stationType, redsk_dim, Befisket_Areal_m2_fun), noSamples = length(unique(sampleId)))
makeTable(dat_tab)

...
cat("# Stationstype (STATTYPE - omdøbes senere)\n")
cat("Stationstype defineres vha. redskabet (gearType), se reference tabellen i Annex 1. Nedestående tabel viser kombinationer i udtrækket")

dat_tab <- summarise(group_by(data, year, cruise, trip, gearType, stationType), noSamples = length(unique(sampleId)))[c(1:10),]
makeTable(dat_tab)
...
cat("# Redskabsdimension (REDSKDIM - omdøbes senere)\n")
cat("Redskabs dimension defineres vha. redskabet (gearType), se reference tabellen i Annex 1. Nedestående tabel viser kombinationer i udtrækket")

tab_dat <- summarise(group_by(data, year, cruise, trip, gearType, stationType, redsk_dim), noSamples = length(unique(sampleId)))[c(1:10),]
makeTable(tab_dat)

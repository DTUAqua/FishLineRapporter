...
gearVal: This may not be needed in the future, since it is only used for specifying the  - *TODO - check*
...
correction_fun: the function for correcting Fangst_kg_m2 (x) for the gear effectiveness e.g. f(x) = x*2.47
...
output_dir <- "Q:/mynd/kibi/fishLine/Rapporter/Biomasseberegninger/creating_a_new_biomasse_report/output/"
...
write.csv(distinct(data, gearType, gearVal, stationType, redsk_dim), paste0(output_dir, "ref_table_original_", Sys.Date(), ".csv"), row.names = F)
...
write.csv(distinct(data, gearType, gearVal, stationType, redsk_dim), paste0(output_dir, "ref_table_corrected_gearVal_", Sys.Date(), ".csv"), row.names = F)

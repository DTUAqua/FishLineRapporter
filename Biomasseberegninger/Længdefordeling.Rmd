...
# gearQuality <- "V"
# redsk_dim <- 1
# areaString <- 'Alle muslingeområder 2011_region'

year<- 2018
cruise <- 'BV LF OES OST april'
species <- "OES"
#trip <- '1 OES' #trip number
gearQuality <- "V"
redsk_dim <- 1
areaString <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/Muslingeomraader/Alle_muslingeområder_2011_region.shp"
...
p_load(sqldf,RODBC,maptools,data.table,rgdal,dplyr,sf)
...
# mus_omr2 <- readShapeSpatial(areaString, 
#                              proj4string=CRS("+proj=longlat +datum=WGS84"))
mus_omr2 <- st_read(areaString) %>%
  st_transform(crs=4326) 

colnames(mus_omr2)[1]<-"Area"
colnames(mus_omr2)[2]<-"AreaName"
# Convert pointsDF to a SpatialPoints object 

data3$x <- data3$lonPosStartDec
data3$y <- data3$latPosStartDec

pointsSP <- st_as_sf(data3, coords = c("x","y"),crs=4326)
# pointsSP <- SpatialPoints(data3[,c("lonPosStartDec","latPosStartDec")], proj4string=CRS("+proj=longlat +datum=WGS84"))

data_omr <- st_join(pointsSP,mus_omr2)
...
#                                latPosStartDec, lonPosStartDec, Area, AreaName,
...
data_table <- data_omr %>%  
  ...
  # select(sampleId, stationType, year, cruise, trip, station, stationName,
  #        latPosStartText,lonPosStartText, latPosStartDec, lonPosStartDec, Area,
  #        AreaName,fishingtime, depthAveGear, Befisket_Areal_m2, areal_metode,
  #        speciesCode, TotalWeight_kg, Length_cm,Number)

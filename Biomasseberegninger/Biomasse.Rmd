...
version_name <- "v14"
output_dir <- "Q:/mynd/kibi/fishLine/FishLineRapporter/Biomasseberegninger/output/"

musOmrString_dir <- "Q:/mynd/kibi/fishLine/FishLineRapporter/Biomasseberegninger/referencer/"
musOmrString <- "Alle_muslingeområder_2019_region_erase2.shp"
...
db_message <- sqlQuery(channel,
                       sprintf("SELECT * FROM ErrorLog
                               WHERE     year in ('", paste(year, collapse = "','"), "') AND
                                         cruise in ('", paste(cruise, collapse = "','"), "')"), 
                                           stringsAsFactors=FALSE)
close(channel)

ref_sample <- distinct(read.csv(paste0(output_dir, "ref_table_BEFISKM2_function.csv"), sep = ";"), gearType, gearVal, STATTYPE, REDSKDIM, 
                       BEFISKM2_function)

ref_species <- rename(distinct(read.csv(paste0(output_dir, "ref_table_KORR_function.csv")), stationType, speciesCode, 
...
FROM     Animal RIGHT OUTER JOIN
                  SpeciesList ON Animal.speciesListId = SpeciesList.speciesListId RIGHT OUTER JOIN
                  Sample ON SpeciesList.sampleId = Sample.sampleId
WHERE  (Sample.cruise LIKE 'BV LF BMS%')", sep = ""), stringsAsFactors = FALSE)
...
FROM     Animal RIGHT OUTER JOIN
                  ...
                  Sample ON SpeciesList.sampleId = Sample.sampleId
                                 WHERE     Sample.year = (",year,") AND
                                           Sample.cruise = ('",cruise,"')", sep=""), stringsAsFactors=FALSE)
...
data$TOTNUM <- data$number*data$raisingFactor
...
data2 <- data1 %>%
  ...
  dplyr::group_by(sampleId, STATTYPE, haulType, gearType, year, cruise, trip,
                  ...
                  REDSKDIM, BEFISKM2, areal_metode, DISMETH, speciesCode, FUNBEFISKM2) %>% 
  dplyr::summarise(TOTWGTSMA = 
                     sum(unique(TOTWGT[sizeSortingDFU=="SMÅ"]),na.rm=T),
                   TOTNUMSMA = 
                     sum(TOTNUM[sizeSortingDFU=="SMÅ"],na.rm=T),
                   ...
                   TOTNUMSTO = 
                     sum(TOTNUM[sizeSortingDFU=="STO"],na.rm=T),
                   LENMEANSTO = 
                   ...
                   TOTNUM = sum(TOTNUM,na.rm=T),
                   LENMEAN = 
                     sum(SOP_Length,na.rm=T)/sum(TOTNUM,na.rm=T),
...
data3 <- mutate(left_join(ungroup(data2), ref_species), KORR_function = 
...
write.csv(ref_species,
          file=paste("ref_table_KORR_function_",
                     format(Sys.time(), "%d%b%Y"), ".csv",sep=""),
          row.names=FALSE,fileEncoding = "UTF-8")
```

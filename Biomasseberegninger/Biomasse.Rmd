...
year <- c(1986, 1996, 1998, 1999)
cruise <- c("BV LF BMS apr", "BV LF BMS maj", "BV LF BMS sept")
species <- "BMS"
...
FROM     Animal RIGHT OUTER JOIN
                  SpeciesList ON Animal.speciesListId = SpeciesList.speciesListId RIGHT OUTER JOIN
                  Sample ON SpeciesList.sampleId = Sample.sampleId
WHERE  (Sample.cruise LIKE 'BV LF OES%')", sep = ""), stringsAsFactors = FALSE)
...
write.csv(final_data,
          file = paste(output_dir, "Biomasse_",
...
write.csv2(final_data,
          file=paste0("Biomasse_", species,"_",
                     format(Sys.time(), "%Y_%m_%d_%H%M"),".csv"), sep="",
          row.names=FALSE)

write.csv2(ref_species,

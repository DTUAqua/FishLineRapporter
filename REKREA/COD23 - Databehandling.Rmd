...
# writeCatch <- "onlyNew"# "all"
# writeTrip <- writeCatch
# writeResp <- "no"
# writeRespCatch <- "no"
# prefix <- ""
...
# csv files with trip information, catch information (both useable for importing data
# to fiskeline), survey responses and a mix of catch and surveyresponses.
# 
# Because data is entered on a tablet in difficult poor conditions, you will need to 
# manually go through the data in order to spot typos and errors. The script contains many
# corrections and as of August 2019, the observers has been instructed in how to correct data
# in Surveygizmo, the current survey system being used.
#
...
# are any changes in the trip data or the responses in the trip, they will not be uploaded.
# 
# Therefore: make sure data is cleaned before importing it to Fiskeline!
# 
# The script creates a small report with some data quality checks, and data for import in
# Fiskeline will also need to go through a data analysis in Fiskeline before it is allowed to be
# imported.
# 
...
exports <- read.csv("Q:/mynd/RekreativtFiskeri/REKREA/Rekrea-programs/Rekrea-programs/data_for_downloader.csv",sep = "\t",stringsAsFactors = F)
...
# This script will load a bunch of functions from get_rekrea_functions.R. If a function fails, 
# the easiest way to see where it fails will be to rename the proper input data in the function 
# and go through the steps on your local machine. Most functions is about connecting 
# different dataset and reshaping data. 

source("get_rekrea_functions_ny.R")
...
age_data0 <- read.csv(paste0(inPath,"ages_191119.csv"),
                      stringsAsFactors = F,na.strings = "", sep = ";")

age_data0$dateGearEnd <- as.POSIXct(strptime(age_data0$dateGearEnd, format="%d-%m-%Y %H:%M"))
age_data0$dateGearStart <- as.POSIXct(strptime(age_data0$dateGearStart, format="%d-%m-%Y %H:%M"))
...
age_data <- subset(age_data,!(sgId == "ON5" & catchNum==1 & age == 4))
...
# ldie removed this because we use trip0 and not trip for the next code - ldie 
...
TRIP <- trip0[trip0$Status=="Complete",-2] %>% 
...
CS$b.spm1_1[CS$Response.ID=="CS670"] <- 6
...
age_data1 <- subset(age_data1, !(sgId == "OF136" & catchNum == 1 & age==4))
age_data1 <- subset(age_data1, !(sgId == "OF46" & catchNum == 2 & age==4))
age_data1 <- subset(age_data1, !(sgId == "OF46" & catchNum == 3 & age==4))
age_data1 <- subset(age_data1, !(sgId == "OF53" & catchNum == 4 & age==4))
# age_data1 <- subset(age_data1, !(sgId == "OF838" & catchNum == 1 & weight = 0.78))
# age_data1 <- subset(age_data1, !(sgId == "OF838" & catchNum == 2 & weight = 0.88))
...
catch_age_data$dateGearEnd <- as.POSIXct(catch_age_data$dateGearEnd, 
                                         origin = "1970-01-01")
catch_age_data$tripRespCatch <- paste(catch_age_data$tripId,catch_age_data$respNum,catch_age_data$catchNum,sep="-")
# overwrite values from surveygizmo with most recent values from fiskeline ----
# in case of changes made in fiskeline after import

ages2 <- ages %>% 
  mutate(catchCategory = ifelse(landingCategory=="KON","C","R"),
         length = length/10,
         tripRespCatch = paste(tripId,respNum,catchNum,sep="-")) #%>% 
  # select(-landingCategory) %>% 
  # rename(species = species)

# added by ldie 30-06-2020
ages2$respNum <- as.numeric(ages2$respNum)

catchInFiskeline <- catch_age_data %>% 
  # mutate(tripRespCatch = paste(tripId,respNum,catchNum,sep="-")) %>% 
  filter(tripRespCatch %in% ages2$tripRespCatch) %>% 
  select(-length,-weight,-age,-number,-code,-species,-catchCategory,
         -otolith,-tripRespCatch, -inAge,-source,-date,-catchNum) %>% 
  distinct()

catchInFiskeline2 <- left_join(catchInFiskeline,ages2) #%>% 
  # distinct()

newCatch <- catch_age_data %>% 
  filter(!tripRespCatch %in% ages2$tripRespCatch) 

catch_age_data2 <- bind_rows(newCatch, catchInFiskeline2) %>% 
  select(-animalId, -individNum, - tripRespCatch) 

# detect respondents with cod catches that they bring home ----
resp_catch_tor <- unique(catch_age_data2[catch_age_data2$species=="TOR" &
                                            catch_age_data2$catchCategory=="C" &
                                            catch_age_data2$number>=0,"sgId"])
...
# changed by ldie den 15-07-2020
...
no_catch_tor <- no_catch_tor %>% 
  mutate(dateGearStart = as.POSIXct(as.Date(date, format="%d/%m/%Y"), startHourTrip, startMinTrip),
         dateGearEnd = as.POSIXct(as.Date(date, format="%d/%m/%Y"),endHourInt,endMinInt))
...
catch_age_data3 <- bind_rows(catch_age_data2, 
                             no_catch_tor %>% select(-startHourTrip, -startMinTrip,
                                                     -endHourInt, -endMinInt))

ca_data_out <- catch_age_data3[,c("sgId","tripId","dfuArea","respNum", "catchNum", 
...
catchFinal <- catch_db_fiskeline1 %>% 
  group_by(sgId) %>% 
  mutate(dateGearEnd = as.POSIXct(max(c(as.numeric(dateGearStart),as.numeric(dateGearEnd)),na.rm=T),
                                  origin = "1970-01-01",tz = "UTC"),
         dateGearStart = as.POSIXct(min(dateGearStart, na.rm=T),
                                    origin = "1970-01-01",tz = "UTC")) %>% 
  ungroup()
...
missingValuesInTrip <- checkMissingValues(trip_db_out[,c("sgTRid","tripId","tripNum")])
missingValuesInCatch <- checkMissingValues(catchFinal[,c("sgId","respNum","sampleType","speciesCode","number","dateGearStart","dateGearEnd","treatment","representative","stepNum","landingCategory","ageMeasureMethod")])


# all trips - trips already in fiskeline will be ignored in the import
if (missingValuesInTrip + missingValuesInCatch >= 0){
  ...
  if (writeTrip %in% c("all","onlyNew")){
    ...
    cat(paste0("\\newline Seneste tur i trip-fil: ",tail(trip_db_out2$tripId,1)))
    
            
        cat("\\subsection{Catch}")
        write.csv2(catchFinal,
...
# class(resp_db1$respNum)<-class(catch_db_out$respNum)
  respcatch <- full_join(catchFinal, resp_db1)
...
catch_db_fiskeline3 <- left_join(catch_db_fiskeline1, trip_db_out) %>% 
...
for (sp in unique(catch_db_fiskeline3$speciesCode)){
  ...
  if (nrow(cf)>0){
    ...
    lap <- ggplot(cf, aes(x=age2, y= length))+
      ...
      geom_point(aes( col=where)) +
      theme_bw() +
      labs(title = sp, x = "Alder", y = "Længde (cm)",col="Datakilde") + 

...
if (!require(sf)) {
  install.packages("sf", repos="http://ftp.ussg.iu.edu/CRAN/")
  library(sf)
}
if (!require(RODBC)) {
...
if (!require(maps)) {
  install.packages("maps", repos="http://ftp.ussg.iu.edu/CRAN/")
  library(maps)
}
if (!require(tidyr)) {
...
if (max_lat-min_lat >2) {
  ...
  ylim <- c(min_lat-.5,max_lat+.5)
  
}

  position$latMid <- (position$lat+position$latEnd)/2
  position$lonMid <- (position$lon+position$lonEnd)/2
...
# bathy <- getNOAA.bathy(xlim[1],xlim[2],ylim[1],ylim[2],resolution = 1,keep = T)
# 
# bathy[bathy>0]<-0
# autoplot.bathy(bathy, geom=c("tile", "contour"), colour=NA, size=0.1)+
...
ia <- st_read(dsn = paste0(ices_areas_string,".shp"))
co <- st_read(dsn= paste0(coast_string,".shp"))

# ia2 <- st_crop(ia, xmin=min(xlim),xmax=max(xlim),
#                ymin=min(ylim),ymax=max(ylim))
# ia2 <- st_intersection(ia,st_set_cr)

set.seed(42)
# autoplot(bathy, geom=c("raster", "contour"), colour="white", size=0.1) + scale_fill_gradient2(low="dodgerblue4", mid="gainsboro", high="darkgreen")



ggplot()+
  geom_sf(data=ia,fill="lightblue",col="grey")+
  geom_sf_text(data=ia,aes(label=KODE))+
  geom_sf(data=co,fill="cornsilk")+
  coord_sf(xlim=xlim, ylim=ylim)+
  geom_segment(data=position, col="black",
            aes(x=lon,y=lat,xend=lonEnd,yend=latEnd,group=station))+
  geom_point(data=position, aes(x=lon,y=lat,fill="start",shape="start"),size=3)+
  geom_point(data=position, aes(x=lonEnd,y=latEnd,fill="end",shape="end"),
             size=3)+
  geom_label_repel(data=position, aes(x=lonMid,y=latMid,label=station),force = 50)+
  scale_fill_manual(values=c("red","green"),
                     name="",
                     labels=c("Station start","Station slut"))+
...
data2[data2$difProc>=20 & !is.na(data2$difProc),"remark"] <- "\\cellcolor{red!50}Stor forskel i vægte"
...
data3 <- rbind(data2,cbind(statError[,-6],totCalc=rep(NA,nrow(statError)),difWeight=rep(NA,nrow(statError)),difProc=rep(NA,nrow(statError)),
                           remark=rep("\\cellcolor{red!50}DTO mangler eller andet problem",nrow(statError) )))
} else {
  data3 <- cbind(statError[,-6],totCalc=rep(NA,nrow(statError)),difWeight=rep(NA,nrow(statError)),difProc=rep(NA,nrow(statError)),remark="\\cellcolor{red!50}DTO mangler eller andet problem" )
...
if (exists("reg_tjek_out") && nrow(reg_tjek_out)>0) {
...
if (any(data3b$landingCategory=="DIS") & exists("indsam")==TRUE) {
  for (i in 1:nrow(indLen3)) {
    if (indLen3$speciesCode[i] %in% indsam$Art & !is.na(indLen3$totNumber.DIS[i]) & is.na(indLen3$numInd.DIS[i]))
        indLen3$numInd.DIS[i] <- "\\cellcolor{red!50}\\it skal indsamles"
  } 
}
...
if (any(data3b$landingCategory=="KON") & exists("indsamKON")==TRUE) {
  for (i in 1:nrow(indLen3)) {
    if (any(data3b$landingCategory=="KON") &&
        ...
        indLen3$numInd.KON[i] <- "\\cellcolor{red!50}\\it skal indsamles"
  }
}
...
if (exists("indLen3") && nrow(indLen3)>0){
...
if (nrow(db_diff)>0) {
  ...
  cat(paste0("I tabellen herunder fremhæves for hver art de indtastede enkeltfisk
    ...
    # db_diff <- db_diff[2:nrow(db_diff),]
      ...
      makeBold <- function(text){return(paste0("\\cellcolor{red!50}", text))}

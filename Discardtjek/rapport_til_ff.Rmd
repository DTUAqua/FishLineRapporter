...
# }
# if (!require(xlsx)) {
#   install.packages("xlsx", repos = "http://ftp.ussg.iu.edu/CRAN/")
#   library(xlsx)
# }
if (!require(gplots)) {
...
# }
if (!require(httr)) {
  install.packages("httr", repos="http://ftp.ussg.iu.edu/CRAN/")
  library(httr)
}
if (!require(reshape2)) {
...
if (!require(rvest)) {
  install.packages("rvest", repos = "http://ftp.ussg.iu.edu/CRAN/")
  library(rvest)
}
if (!require(stringr)) {
...
if (!require(pdftools)) {
  install.packages("pdftools", repos = "http://ftp.ussg.iu.edu/CRAN/")
  library(pdftools)
}
...
# areas <- c(@paramAreas)
# logbook0 <- read_sas("BMSlog.sas7bdat")
# landingDecPath0 <- read_sas("BMStable.sas7bdat")
...
# This part reads a list of members. Only discardtrips with members should be included (gdpr...)
...
# 
# ffurl <- "https://fiskeriforening.dk"
# omos <- "/om-os/vi-er/fiskere-og-fartoejer/"
# 
# # a <- 
# 
# urls <- html_session(paste0(ffurl,omos),ua) %>% 
#   read_html() %>% 
#   html_nodes("a") %>% 
#   html_attr("href")
# url <- paste0(ffurl,grep("kutterliste.pdf",urls,value = T))
# 
# ffmembers <- pdftools::pdf_text(url)
# memberboats <- c()
# 
# for (page in 1:length(ffmembers)) {
#   
#   members <- strsplit(split = "\r\n",
#                       x = sub(x = ffmembers[page],
#                               pattern = ".*[Ss]uspenderet\r\n(.+)",
#                               replacement = "\\1"))
#   
#   members2 <- sub(members[[1]],pattern = " {2,}.+",replacement = "" )
#  
#   memberboats <- c(memberboats,members2) 
# }
...
query <- sprintf("SELECT s.[year]
  ...
  year, startOfQuarter, startNextQuarter)#,paste0(memberboats,collapse="','"))
...
speciesDFAD <- read.csv("Q:/mynd/SAS Library/Arter/art.csv")
        ...
        mutate(
          landingCat =
            ifelse((landingCategory %in% konsum_cats &
              ...
              year == year), "Landinger",
            ifelse((year == year &
                      landingCategory %in% discard_cats &
                      cruise %in% c("SEAS", "MON")), "Discard",

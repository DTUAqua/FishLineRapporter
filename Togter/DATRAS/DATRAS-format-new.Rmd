...
cruise_all <- c("Muslingetogt")
v_cruise <- c("BV LF BMS mar", "BV LF OES OST mar")
v_year <- c("2024")
art_out <- c("BMS")
...
if (any(toupper(v_cruise) %in% c("TUNGER", "Tunger"))) {survey <- "TN" }
...
if (survey %in% c("BITS","NS-IBTS","KASU")) {
  ...
  # ldie - ændret den 04/04/2022
} else if (survey %in% c("TBM")) {
...
if (survey %in% c("TBM")) {
  hh2$ship[hh2$ship == "REYKJA"] <- "26RK"
  hh2$ship[hh2$ship == "SALLIN"] <- "26SB"
  hh2$ship[hh2$ship == "L526"] <- "26BV"
  hh2$ship[hh2$ship == "ARCTIC"] <- "26G1"
...
for (i in 1:nrow(hh2)) {
  ...
  if (hh2$survey[i]=='BITS' & hh2$gearType[i]=='TV3' & !is.na(hh2$gearType[i]) ) {
    ...
    # Tobis skrabetogt  # ldie - tilføjet 04/04/2022
  } else if (hh2$survey[i]=='TBM' & hh2$gearType[i]!="GRB" & !is.na(hh2$gearType[i])) {
    hh2$Gear[i] <- 'DSD' ; hh2$SweepLngt[i] <- -9; hh2$GearEx[i] <- "S";
    hh2$DoorType[i] <- -9; hh2$WarpDen[i] <- -9; hh2$Warpdia[i] <- -9;
    hh2$DoorSurface[i] <- -9; hh2$DoorWgt[i] <- -9; hh2$Buoyancy[i]<- -9;
    hh2$KiteDim[i] <- -9; hh2$WgtGroundRope[i] <- -9
...
hl3 <- sqldf('select   sampleid, specieslistid, SpecCodeType, SpecCode, SpecVal,
             Sex, DevStage, LenMeasType, CatIdentifier, representative, SubFactor, SubWgt,
             LngtCode, length as LngtClass, sum(Animal_number) as HlNoAtLngt
             from     hl2
             group by sampleid, specieslistid, SpecCodeType, SpecCode, SpecVal, Sex, DevStage, LenMeasType,
...
hl4_a <- sqldf('select   sampleid, specieslistid, SpecCodeType, SpecCode, SpecVal, Sex, DevStage, LenMeasType,
...
ca$FishID <- as.numeric(ifelse_na(ca$animalId))
...
# ldie - changed the 10-02-2022 # fra A,B,C,D til AQ1, AQ2, AQ3 (mangler en AQ4)
ca$OtGrading <- ifelse(is.na(ca$otolithReadingRemark), -9,
...
ca$Comments <- ifelse(!is.na(ca$remark.1) & !is.na(ca$remark), paste(ca$remark.1,ca$remark, sep = "/"), 
                      ifelse(!is.na(ca$remark.1) & is.na(ca$remark), paste(ca$remark.1),
                             ifelse(!is.na(ca$remark) & is.na(ca$remark.1), paste(ca$remark),-9)))
...
# No Maturity
ca[!(ca$maturityIndexMethod %in% c("A","B","C","I", "N", "M")) |
...
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 21, "Maturity"] <- 62
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 22, "Maturity"] <- 62
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 31, "Maturity"] <- 63
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 32, "Maturity"] <- 63
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 41, "Maturity"] <- 64
ca[ca$maturityIndexMethod == 'N' & ca$maturityIndex == 42, "Maturity"] <- 64
...
ca_age <- sqldf('select   sampleId, FishID, speciesListId, SpecCodeType, SpecCode,
                          SpecVal, Sex_CA, CatIdentifier, LngtCode, 
                          length as LngtClass, PlusGr, age as AgeRings, 
                          Maturity, MaturityScale, sum(age_number) as CANoAtLngt, representative, 
                          GenSamp, AgeSource, AgePrepMet, OtGrading, OtolithsAvailable,
                          StomSamp, Comments, ParSamp, LiverWeight, GonadWeight, GuttedWeight, 
                          LiverWormScaleUsed, LiverParasiteStage
                 from     ca
                 group by sampleId, FishID, speciesListId, SpecCodeType, SpecCode, SpecVal, 
                          Sex_CA, CatIdentifier, representative, GenSamp, AgeSource, AgePrepMet, OtGrading, OtolithsAvailable,
                          StomSamp, Comments, ParSamp,LiverWeight, GonadWeight, GuttedWeight, 
                          LiverWormScaleUsed, LiverParasiteStage,
                          LngtCode, LngtClass, PlusGr, age, Maturity, MaturityScale')
# LngtCode, length, PlusGr, age, Maturity')

# sum total number of fish and weight per length
ca_wgt <- sqldf('select   sampleId, FishID, speciesListId, SpecCodeType, SpecCode, SpecVal, 
                          Sex_CA, CatIdentifier, LngtCode, length as LngtClass,
                          PlusGr, age as AgeRings, Maturity, MaturityScale, representative, 
                          GenSamp, AgeSource, AgePrepMet, OtGrading, OtolithsAvailable, StomSamp,
                          Comments, ParSamp, LiverWeight, GonadWeight, GuttedWeight, 
                          LiverWormScaleUsed, LiverParasiteStage,
                          sum(weight*1000)/sum(Animal_number) as IndWgt
                 from     ca
                 where    weight not like "NA"
                 group by sampleId, FishID, speciesListId, SpecCodeType, SpecCode, SpecVal, 
                          Sex_CA, CatIdentifier, LngtCode, representative,GenSamp, AgeSource, AgePrepMet, OtGrading,
                          OtolithsAvailable, StomSamp, 
                          Comments, ParSamp, LiverWeight, GonadWeight, GuttedWeight, 
                          LiverWormScaleUsed, LiverParasiteStage,
                          length, PlusGr,age, Maturity, MaturityScale')


ca3 <- merge(ca_age, ca_wgt, by = c("sampleId", "FishID", "speciesListId","representative", "GenSamp", 
                                    "AgeSource", "AgePrepMet","OtGrading", "OtolithsAvailable", "StomSamp", "Comments", 
                                    "ParSamp", "LiverWeight",
                                    "GonadWeight", "GuttedWeight", "LiverWormScaleUsed", "LiverParasiteStage",
                                    "SpecCodeType", "SpecCode", "SpecVal", "Sex_CA",
                                    "CatIdentifier", "LngtCode", "LngtClass",
                                    "PlusGr", "AgeRings", "Maturity", "MaturityScale"), all = TRUE)
...
if (cruise_all %in% c("BALTICA_parasite")) {
  ...
  DATA_CA <- data.frame("RecordType"="CA","Quarter"=-9,
...
DATA_HL <- data.frame(
  ...
  "LenMeasType" = hhhlca3$LenMeasType[in_HL],
  stringsAsFactors = FALSE)

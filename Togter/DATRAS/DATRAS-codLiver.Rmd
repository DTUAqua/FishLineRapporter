...
#
# cruise_all <- c("BITS")
# v_cruise <- c("BITS-1","BITS-2", "BALTICA", "KASU-1", "KASU-2")
# # cruise_all <- c("TBM")
# # v_cruise <- c("Tobisskrabetogt LH.")
# v_year <- c("2017","2018", "2019")
# art_out <- c("TOR")
...
hlca_a <- sqlQuery(channel, paste0(
  ...
  an.parasiteCode, an.remark,
  an.weightLiver, an.weightGonads, an.weightGutted, an.animalId,
...
hlca_a <- merge(hlca_a, hlca_a2, by = "animalId", all.x = T)
...
ca$LiverWeight <- ca$weightLiver
ca$GonadWeight <- ca$weightGonads
ca$GuttedWeight <- ca$weightGutted
...
# weight of liver, gonad and gutted weight in gram ------------------------------------ 
...
# otoliths - no data so far so everything is set to -9 -------------------
...
#ca$StomachFullness <- ifelse(ca$StomachFullness %in% c("0", "2"), c("0", "2"), "-9")
ca$StomachFullness <- ifelse(ca$StomachFullness %in% c("0", "2"), ca$StomachFullness, "-9")

# Comments  ----------------------------------------------------------------------------------------------
ca$Comments <- paste(ca$an_remark,ca$remark, sep = "/")
...
# MaturityScaleUsed ldie
ca$MaturityScaleUsed[ca$maturityIndexMethod=="I"] <- "I"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="C"] <- "Tomkiewicz"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="A"] <- "A"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="B"] <- "B"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="M"] <- "M"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="N"] <- "N"
ca$MaturityScaleUsed[ca$maturityIndexMethod=="U"] <- "U"
...
ca_age <- sqldf('select   sampleId, speciesListId, SpecCodeType, SpecCode,
                          ...
                          length as LngtClass, PlusGr, age as AgeRings, Maturity, sum(age_number) as CANoAtLngt, Comments,
                          representative, animalId, LiverWormScaleUsed, StomachFullness, LiverParasiteStage, GuttedWeight, LiverWeight, GonadWeight
                 from     ca
...
if (nrow(ca)>1) {
  DATA_CA <- data.frame(
    ...
    "animalId" = hhhlca3$animalId[in_CA],
    "LiverWormScaleUsed" = hhhlca3$LiverWormScaleUsed[in_CA],
    "LiverParasiteStage" = hhhlca3$LiverParasiteStage[in_CA],
    "GuttedWeight" = hhhlca3$GuttedWeight[in_CA],
    "LiverWeight" = hhhlca3$LiverWeight[in_CA],
    "GonadWeight" = hhhlca3$GonadWeight[in_CA],
    "StomachFullness" = hhhlca3$StomachFullness[in_CA],
    "OtolithsAvailable" = hhhlca3$OtolithsAvailable[in_CA],
    "Comments" = hhhlca3$Comments[in_CA],
    stringsAsFactors = FALSE)
} else {
  DATA_CA <- data.frame("RecordType"="CA","Survey"=-9,"Quarter"=-9,
                        ...
                        "ValidAphiaID"=-9, "animalId"=-9, "LiverWormScaleUsed"=-9, "LiverParasiteStage"=-9, 
                        "GuttedWeight"=-9, "LiverWeight"=-9, "StomachFullness"=-9, "OtolithsAvailable"= -9, 
                        "GonadWeight"=-9, "Comments"=-9, stringsAsFactors = FALSE)
}
...
# adding a unique number identifying the cruise 
DATA_CA$CruiseNumber <- paste(DATA_CA$Survey, DATA_CA$Quarter, DATA_CA$Country, sep = "_" )
...
#path <- "Q:\\dfad\\users\\ldie\\data\\DATRAS\\" 
path <- ""

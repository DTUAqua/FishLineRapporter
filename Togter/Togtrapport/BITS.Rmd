...
p_load(sqldf, RODBC, ggplot2, reshape, plyr, data.table, 
       ...
       gplot, Hmisc, stringr, rgdal, maptools, shapefiles,
       calibrate,plotrix,base,mapplots)
#dismo, raster, marmap, rasterVis,
...
# if (method_run == "R") {
    ...
    # str_other  <- "Text"
    # 
    # participantsString <- "C:/Users/trkj/Downloads/participants.csv"
...
# } else if (method_run == "FiskeLine") {
  ...
  participantsString <- "participants.csv"
...
channel <- odbcConnect("DanaDB_DW") # OR "DanaDBDW"/"DanaDB_DW"
...
participants <- read.csv.unknown.sep(participantsString,4)  
...
Wind speed and direction are presented in Fig. \ref{fig:wind}. 

Number of days with an average wind speed larger than 15 m/s: `r L15wind_days`.
...
if (yn_guest == "yes"){
  ...
  cat("\\subsection{Guests on board} ")
  
  cat(paste0("There is a long and strong tradition to have scientific colleagues from other countries onboard in exchange during the survey. The reason for that is the facilitating of cooperation and standardization of procedures across participating countries. Guests on this survey: ",str_guest))

...
wind <- wind[wind$date > (start_date -2) & wind$date < (end_date  + 2)]
...
```
Wind speed and direction are presented in the end of the report in Figure 5. The   
...
bongo <- stations[!is.na(stations$status) & stations$gearCode=="BONGO" & stations$status != "Fail", ]
```
...
ylim <- c(54,56.5)
xlim <- c(14, max(samples$lonPosStartDec))

a <- data.frame(timeStart = samples$haul_start, lon = samples$lonPosStartDec, lat = samples$latPosStartDec)
a$gearCode <- "Trawl"

if(exDate != "No exchange"){
b <- data.frame(timeStart = as.POSIXct(exDate), lon = 14.69, lat = 55.10, gearCode = "Harb")
} else {
  b <- c()
}

q <- rbind(a, b, bongo[, c("timeStart", "lon", "lat", "gearCode")])

q$gearCode <- as.numeric(as.factor(q$gearCode))
q <- q[order(q$timeStart), ]
...
p1 <- basemap(xlim, ylim, main = paste0(titl, " ",  quarter, "Q ", year))+
  ...
  draw.rect()+
  points(q$lon, q$lat, col= 1 + q$gearCode, pch = 16, cex = 0.75)
  legend("bottomright", legend=c("Trawl ", "Bongo"),
         col=c("green", "red"), pch = 16, cex=1.5, bg='white')
...
apt <- stations[stations$gearCode=="appi" & stations$status!="Fail",] 
...
Figure 5, shows the Wind speed and wind direction along the cruise track

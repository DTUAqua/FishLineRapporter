...
p_load(data.table, stringr, sf, mapplots, shapefiles, maptools, ggplot2, tidyr, lubridate, car)
...
# cruise <- "MON"
# tripId <- "3166"
# coast_string <- "Q:/20-forskning/12-gis/Dynamisk/GEOdata/BasicLayers/CoastLines/Europe/europe_simple_wgs84"
# dfad <- readRDS("//aqua-pkfweb21.win.dtu.dk/dfad-share/dfad.rds")
# sel <- read.csv2("//aqua-pkfweb21.win.dtu.dk/dfad-share/rdsk_select.csv")
...
trip <-sqlQuery(channel, paste("SELECT    
  ...
  WHERE     year IN ('",paste0(year,collapse="','"), "') AND
            ...
            trip IN ('",paste0(tripId,collapse="','"), "') 
  " ,sep = ""))
...
dbErrors <- sqlQuery(channel, paste("SELECT
  ...
  where origin like '",year,"%",cruise,"%",tripId,"%'
" ,sep = ""))
...
if (logbook_match != "TRUE") {
  ...
  #selections panel
  panel <- sel[sel$nummer %in% dfad$redskstr, ]$Paneltype
  
  if (identical(panel, character(0)))
    panel <- "Ugyldig panel kode"
...
tracks$dist_nm <- as.numeric(st_length(tracks))*0.000539956803
tracks <- st_buffer(tracks, 0.0001)
...
if (TRUE %in% tracks$dist_nm > 10){
  
  "Meget langt slæb på en station, tjek start og slut koordinaterne" %>% 
      cell_spec(color = "red", font_size = 15, bold = T) %>%
      kbl(booktabs = T, escape = F, col.names = NULL)  %2>%  
      kable_styling(latex_options = c("hold_position")) 
}
...
if (nrow(data.frame(sp_lst)) > 0) {
  ...
  est[is.na(est$totalWeight)] 
  #sumerize acording to sample level
  est <- est[est$speciesCode != "DTO"]
  
    
  est <- est[ ,. (tast = round(sum(weightStep3 * treatmentFactor 
                                   * raisingFactor, na.rm = T), 1),
  ...
  est$bem <- ifelse(is.infinite(est$tot), "Ingen total vægt", est$bem)
  ...
  est$bem[est$bem == "Ingen total vægt"] <- cell_spec(est$bem[est$bem == "Ingen total vægt"],
...
names(animal)[names(animal) == "speciesCode2"] <- "speciesCode"

units <- unique(rbind(animal[ , c("speciesCode", "lengthMeasureUnit")],
                      lw_old[lw_old$year == year-1 , c("speciesCode", "lengthMeasureUnit")]))
units <- units[units$speciesCode %in% 
                 units$speciesCode[duplicated(units$speciesCode)], ]

if (nrow(units) > 0){
  
  cat(paste0("\n\n### Tjek længdemålings enheden i følgende arter \n"))
  
  toString(unique(units$speciesCode)) %>% 
     cell_spec(color = "red", font_size = 10, bold = T) %>%
     kbl(booktabs = T, escape = F, col.names = NULL)  %>%  
     kable_styling(latex_options = c("hold_position"))
...
if (nrow(ejrep) > 0){
  ...
  print(ejrep[, 1:8] %>%
         kbl(escape = F, longtable = T, caption = "Manglende rep Længder", 
             ...
             col.names = c("Station", "Art", "Landings-kategori", 
                           "sort", "længde", "individNr", "type", "type2"), 
             align = "l") %>%
         row_spec(0, bold = TRUE) %>%
...
fun <- function(i) {
 ...
 plot(old$length, old$weight, pch = 16, cex = 0.1, col = "white",
        ...
        cex.main = 1.5,
        cex.lab = 1.5,
        xlim = c(range(c(old$length, new$length))[1], range(c(old$length, new$length))[2]),
        ylim = c(range(c(old$weight, new$weight))[1], range(c(old$weight, new$weight))[2]))+

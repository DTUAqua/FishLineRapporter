...
animal <- merge(animal, age, by = "animalId", all.x = T)
...
dfad <- dfad[dfad$fid %in% dat$platform1, ]
...
if (logbook_match != "TRUE") {
  ...
  tbl <- rbind(data.frame(a = "Fartøj", 
               ...
               data.frame(a = "Selektionsudstyr", 
                          ...
                          c = sel[sel$Anføres.med.nummer %in% dfad$redskstr, ]$Paneltype))
...
p1 <- basemap(xlim, ylim, main = "Stationer")+
              ...
              draw.rect()+
              segments(x0=samples$lonPosStartDec, y0=samples$latPosStartDec, 
                       x1=samples$lonPosEndDec, y1=samples$latPosEndDec,
                cex=0.8, col="darkblue")
              points(samples$lonPosStartDec, samples$latPosStartDec,  pch=16, col="red", cex=1.5)
              points(samples$lonPosEndDec, samples$latPosEndDec,  pch=24, col="black",bg="green")
...
#### points on land
pts_start <- samples[, c("station", "lonPosStartDec", "latPosStartDec")] %>% 
        sf::st_as_sf(coords = c("lonPosStartDec","latPosStartDec")) %>% 
        sf::st_set_crs(4326)

pts_end <- samples[, c("station", "lonPosEndDec", "latPosEndDec")] %>% 
...
if (nrow(data.frame(sp_lst)) > 0) {
  ...
  est <- data.table(sp_lst[sp_lst$speciesCode != "DTO", ])
  ...
  est <- merge(est, beregnet, by = "sampleId")
  est$pct <- round(((est$calc - est$tast) / est$calc)*100)
  
  est$bem <- ifelse(est$pct < 20 & est$pct > - 20, " Vægte ok", "Stor forskel i vægte")
...
sp_lst$spørgsmål <- ifelse(sp_lst$landingCategory %in% c("SLP", "IND", "BMS"), 
                       pasye0("Der er oplysning om",  sp_lst$landingCategory, "på artslisten - er det korrekt?"), 
                              "OK") 
...
`r ifelse(unique(tbl1$samp_lvl) != "OK", tbl1[, c("station", "samp_lvl")], "OK")`

\vspace{0.5cm}
\underline{Fejl I "Discard" Registreing}\newline
`r ifelse(unique(tbl1$disck) != "OK", tbl1[, c("station", "disck")], "Ingen Fejl")`

\vspace{0.5cm}
\underline{Fejl I "Landings" Registreing}\newline
`r ifelse(unique(tbl1$land) != "OK", tbl1[, c("station", "land")], "Ingen Fejl")`

\vspace{0.5cm}
\underline{Fejl I "Nul fangsts" Registreing}\newline
`r ifelse(unique(tbl1$no_catch) != "OK", tbl1[, c("station", "no_catch")], "Ingen Fejl")`

\vspace{0.5cm}
\underline{Spørgsmål Til "Alt Registreret"}\newline
`r ifelse(unique(tbl1$all_all) != "OK", tbl1[, c("station", "all_all")], "Ingen Spørgsmål")`

\vspace{0.5cm}
\underline{Spørgsmål Til "Landings kattegori"}\newline
`r ifelse(unique(tbl1$spørgsmål) != "OK", tbl1[, c("station", "spørgsmål")], "Ingen Spørgsmål")`
...
```{r ,echo=FALSE, cache=FALSE,message=FALSE, warning=FALSE,results='asis',fig.width=15, fig.height=12, fig.pos="ht!"}
  ...
  if (TRUE == unique(sp_lst_rais$landingCategory %in% c("KON", "DIS"))){

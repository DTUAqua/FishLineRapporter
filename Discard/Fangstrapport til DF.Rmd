...
p_load(sqldf,RODBC,plyr,dplyr,stringr,xtable,ggmap,gplots,grid,ggrepel,reshape2,
       marmap,sf,ggrepel,lwgeom,shapefiles,maptools,mapplots,wordcloud,scales)
...
##
 channel <- odbcConnect("FishLineDW")
  multiplot <- sqlQuery(channel, sprintf("select distinct speciesCode from [FishLineDW].[dbo].animal where year = %s and cruise = '%s' and trip = '%s' ",year,cruise,trip),stringsAsFactors=FALSE)
...
hilsen <- ""
coast_string <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/CoastLines/EuropeWGS84.shp"
dk_eez_string <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/EEZ/eez_dk_line.shp"
ices_areas_string <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/Ices/ICES_areas.shp"
ices_sq_string <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/Boundaries/Ices/ICES_quarter_squares.shp"
...
SpeciesListRaised.speciesListRaisedId as ID
   FROM         [FishLineDW].[dbo].SpeciesListRaised INNER JOIN
                      [FishLineDW].[dbo].Sample ON [FishLineDW].[dbo].SpeciesListRaised.sampleId = Sample.sampleId
...
query3b<-paste("SELECT SpeciesList.trip, SpeciesList.year,SpeciesList.station,  SpeciesList.speciesCode, 
                ...
                FROM   [FishLineDW].[dbo].SpeciesList INNER JOIN
                       [FishLineDW].[dbo].Sample ON [FishLineDW].[dbo].SpeciesList.sampleId = Sample.sampleId
...
"Skagerrak" = c(NA,30,NA,NA,27,30,18,NA,20,10.5,27,30,60,24,40,NA,23),
"Kattegat" = c(NA,30,NA,NA,27,30,18,NA,20,10.5,27,30,60,24,40,NA,23),
"Nordsøen" = c(NA,35,NA,NA,30,27,20,NA,30,8.5,27,35,60,24,40,NA,27),
"Vestlige_Østersø" = c(30,35,NA,23,NA,NA,NA,NA,NA,10.5,25,NA,60,NA,40,30,NA),
"Østlige_Østersø" = c(30,35,NA,23,NA,NA,NA,NA,NA,10.5,25,NA,60,NA,40,30,NA),
note = NA)
...
catchOperation3 <- unique(paste(toupper(substr(catchOperation2, 1, 1)), substr(catchOperation2, 2, nchar(catchOperation2)), sep=""))
...
# ices_sq <- st_read(ices_sq_string) %>% st_transform(crs=4326)  
  ...
  coast <- read.shapefile(substr(coast_string,0,nchar(coast_string)-4))
  ices_areas <- read.shapefile(substr(ices_areas_string,0,nchar(ices_areas_string)-4))
  map <- NULL
```
...
ylim <- c(floor(min(c(position$lat,position$latEnd),na.rm=T)),
          ceiling(max(c(position$lat,position$latEnd),na.rm=T)))
xlim <- c(floor(min(c(position$lon,position$lonEnd),na.rm=T)),
          ceiling(max(c(position$lon,position$lonEnd),na.rm=T)))
  
pos <- matrix(c(position$lon,position$lat,position$lonEnd,position$latEnd,
                position$lonMid,position$latMid),
              ncol=6,
              dimnames = list(position$station,NULL))
...
basemap(xlim=xlim, ylim=ylim, main = "",
        xlab=expression("Grader østlig længde"),
        ylab=expression("Grader nordlig bredde"),bg = "lightblue" ) #, bg="white")
draw.rect(col="grey")
...
# wordcloud::textplot(x=pos[,5], y=pos[,6],words = row.names(pos),
#                     show.lines = T,new = F)
...
`r if(exists("data5") && !is.character(data5) && !is.na(data5[1,1])==TRUE) {capwords(toString(data5[1,1]), strict=TRUE)}` \
`r if(exists("data5") && !is.character(data5) && !is.na(data5[1,2])==TRUE) {capwords(toString(data5[1,2]), strict=TRUE)}` \
`r if(exists("data5") && !is.character(data5) && !is.na(data5[1,3])==TRUE) {capwords(toString(data5[1,3]), strict=TRUE)}` \
...
finalData_tab3$weightTotal.DIS[!(finalData_tab3$station %in% station_list) & finalData_tab3$weightTotal.DIS == "0,0"] <- "Ej oparbejdet"
#finalData_tab3$weightTotal.KON[!(finalData_tab3$station %in% station_list) & finalData_tab3$weightTotal.KON == "0,0"] <- "Ej oparbejdet"
...
for (i in input_art) {
  ...
  if (i %in% plot_spec2$speciesCode) {
    ...
    if (nrow(dat.melt[dat.melt$number > 0, ]) > 0){

...
quarter <- 3
cruise <- 'MON'
mappath <- "Q:/gis/Dynamisk/GEOdata/BasicLayers/CoastLines/EuropeWGS84"
...
p_load(sqldf,RODBC,plyr,xtable,gplots,grid,reshape2,ggrepel,gridExtra,gplots,Hmisc,
       stringr,dplyr,data.table,mapplots,base,plotrix,calibrate,shapefiles,
       rasterVis,maptools,rgdal,marmap,dismo,raster,Hmisc,colorspace)
...
gear_input[gear_input$dfuArea %in% c(21) & 
             !(gear_input$selectionDevice %in%
                 c("Seltra 300","Topløs/seltra >175mm","Seltra 140mm Kvardrat ",
                   "4 panel/270 diamant")) & 
             gear_input$gearType %in% c("OTB","OTT","PTB"),"dfuArea"] <- 
  paste("\\color{red}",
        gear_input[gear_input$dfuArea %in% c(21) &
                     !(gear_input$selectionDevice %in% 
                         c("Seltra 300","Topløs/seltra >175mm",
                           "Seltra 140mm Kvardrat ","4 panel/270 diamant")) & 
                     gear_input$gearType %in% c("OTB","OTT","PTB"),"dfuArea"])
...
mis_gear2 <- mis_gear2[! mis_gear2$gearType %in% c('GTR','GNS','GND','GN','GTN', 'MSP', 'GNC'), ]
